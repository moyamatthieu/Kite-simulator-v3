 ...
 */
export interface IGroundCollisionSystem {
...
export interface IBoundaryConstraints {
    applyBoundaryConstraints(kite: THREE.Object3D, velocity: THREE.Vector3): void;
    setBounds(bounds: Partial<{
        minX: number; maxX: number; minY: number; maxY: number; minZ: number; maxZ: number;
    }>): void;
    isAtBoundary(kite: THREE.Object3D): boolean; // Ajout de la méthode isAtBoundary
}
...
export interface ILineConstraints {
    ...
    getMaxLengths(): { left: number; right: number };
}

export interface IPhysicsConstraints {
    ...
    groundSystem: IGroundCollisionSystem;
    boundarySystem: IBoundaryConstraints;
}

export interface ILineSystem {
    // Les méthodes de ILineSystem seront déplacées dans ILineConstraints ci-dessus ou seront redefinis.
    // L'interface ILineSystem redéfinit "enforceConstraints" qui est déjà utilisé.
    // Pour l'instant, je vais adapter les méthodes nécessaires dans LineConstraints.
    // update(handles: IHandlePositions, kitePoints: IKitePoints): ILineTension;  // Cette methode sera déplacée ou adaptée
    // getLinePoints(): ILineRenderData[];  // Cette methode sera déplacée ou adaptée
}
